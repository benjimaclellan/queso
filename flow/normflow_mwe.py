...
from flow.models import RNN, Dense, Flow
...
def sample(phase: float):
    p = np.array([np.cos(n_qubits * phase) ** 2, np.sin( n_qubits * phase) ** 2])  # GHZ state
    p = p / np.sum(p)
...
def sequence(shots: torch.Tensor, phases: torch.Tensor, n_seq: int):
    ...
    shots_r = torch.Tensor(einops.rearrange(shots_r, "b (bp seq) q -> (b bp) seq q", bp=batch_phase)).to(device)

    phases_r = torch.Tensor(einops.repeat(phases, "b -> (b bp) 1", bp=batch_phase)).to(device)
...
for n_seq in (n_train_seq,):
    ...
    for i in range(max_iter):
        ...
        if progress:
            pbar.update()
            pbar.set_description(f"Step {i} | Loss: {loss:.10f} | sequence length {n_seq}", refresh=False)
...
for i, n_seq in enumerate([2, 20, 50, 100]):
    ...
    ax.scatter(phases_r.cpu().squeeze(), z.cpu().detach().squeeze(), alpha=0.1, label=f"infer, sequence length $m={n_seq}$")
    ax.set(ylim=[0, torch.pi/2/n_qubits])
    ax.legend()
axs[0].set(title=f"train, sequence length, $m={n_train_seq}$")
axs[-1].set(xlabel=r'Phase (ground truth), $\varphi$', ylabel=r'Estimated phase, $\bar{\varphi}$')
fig.show()
...
for i, n_seq in enumerate([2, 20, 50, 100]):
    ...
    axs[i, 0].scatter(phases_r.cpu().squeeze(), latent[:, 0].cpu().detach().squeeze(), alpha=0.1)
    axs[i, 1].scatter(phases_r.cpu().squeeze(), latent[:, 1].cpu().detach().squeeze(), alpha=0.1)
    # ax.set(ylim=[0, torch.pi/2/n_qubits])

axs[-1, 0].set(xlabel=r'Phase (ground truth), $\varphi$', ylabel=r'Latent var., $\lambda$')
axs[-1, 1].set(xlabel=r'Phase (ground truth), $\varphi$', ylabel=r'Latent var., $\sigma$')
fig.show()
...
n_seq = 100
shots_t, phases_t = sequence(shots[None, n_phases//2, :, :], phases[None, n_phases//2], n_seq=n_seq)
