...
# for n_ancilla in (0, 1, 2, 3, 5, 6):
    ...
    config.n_phis = 200
    config.phi_center = 0.0
    config.phi_range = [-pi / 2 + config.phi_center, pi / 2 + config.phi_center]

    config.phis_test = (np.linspace(-pi/3, pi/3, 6) + config.phi_center).tolist()  # [-0.4 * pi, -0.1 * pi, -0.5 * pi/n/2]
